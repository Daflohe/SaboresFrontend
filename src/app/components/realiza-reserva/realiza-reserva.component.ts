...
export class RealizaReservaComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,public api:ApiService) {
    ...
    onSubmit(): void {
      const hora = this.addressForm.get('hora')?.value
      var arrayDeCadenas = hora.split(":");

      const horas= arrayDeCadenas[0]
      const minutos= arrayDeCadenas[1]
    const reserva={
      idUsuario: this.user.idUsuario,
      idSede: this.addressForm.get('sede')?.value,
      fechaReserva:this.addressForm.get('fecha')?.value+"T00:00:00",
      cantidadPersonas: parseInt(this.addressForm.get('cantidad')?.value),
